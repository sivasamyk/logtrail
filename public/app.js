...
uiModules
    .get('app/auth_logtrail')
    .controller('comController',function ($scope, kbnUrl, $route, $routeParams,
      ...
      //세션값 체크
      $http.get(chrome.addBasePath('/auth_logtrail/checkSession')).then(function (res) {
        if( res.data.session ) {
          //세션 있으면 화면 로딩
          init();
        } else {
      ...
      function init() {
        //설정파일 로딩
        $http.get(chrome.addBasePath('/auth_logtrail/config')).then(function (resp) {
          if (resp.data.ok) {
      ...
      function doSearch(rangeType,order,actions,timestamp) {
        ...
        return $http.post(chrome.addBasePath('/auth_logtrail/search'), request).then(function (resp) {
          if (resp.data.ok) {
      ...
      */
      ...
      $scope.logout = function(){
        stopTailTimer();
        $http.get(chrome.addBasePath('/auth_logtrail/logout')).then(function (res) {
          $location.path('/login');
        });
      };
      ...
      function setupHostsList() {
        ...
        $http.post(chrome.addBasePath('/auth_logtrail/hosts'),params).then(function (resp) {
          if (resp.data.ok) {
            $scope.hosts = [];
...
uiModules.get('app/auth_logtrail').directive('onLastRepeat', function () {
...
uiModules.get('app/auth_logtrail').directive('clickOutside', function ($document) {
  return {
    restrict: 'A',
    scope: false,
...
uiModules.get('app/auth_logtrail').filter('ansiToHtml', function ($sce) {
...
uiModules.get('app/auth_logtrail').directive('compileTemplate', function ($compile, $parse) {
  return {
...
uiModules
    ...
    .controller('loginController',function ($scope,$http,$location) {
      ...
      $scope.login = function login() {
        ...
        $http.post(chrome.addBasePath('/auth_logtrail/login'), {id:id,pw:pw}).then(function (resp) {
